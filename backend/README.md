...
**A report is an aggregate over the scoped query, and only shared answers are cached.** Attendance reports group the exact query the record list runs, tenant predicate and access scope included, so a report cannot count a row its caller could not read. Whole-school reports are cached in Redis; a report the scope narrows, a teacher's or a parent's, is computed every time and never written to Redis, so no key can hold an answer that belongs to one caller. Cached reports carry a per-school generation that every attendance write moves on after it commits, and a reader takes the generation before it reads Postgres, so a slow reader that raced a write can only store its stale answer where nobody looks. Redis is an optimisation: if it is down, reports come from Postgres and writes still succeed.
